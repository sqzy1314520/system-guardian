...
updated: 2026-06-16
changelog:
  - 1.16.0: path-standards.md 新增「标准依据」节，引用 Issue #892/#8669 确认 expanduser 为社区标准；home-override-fix.md 统一到社区标准模式；新增系统性扫描方法论参考；compliance-check.sh 新增第13项「路径写法合规」自动扫描
  - 1.15.3: path-standards.md 新增「标准依据」节，引用 Issue #892/#8669 确认 `expanduser("~/.hermes")` 为社区标准 fallback；修正 changelog 1.15.1 错误表述（expanduser 不是 bug，是标准）
  - 1.15.2: 更新 home-override-fix.md — 新增四层系统扫描方法论、修复优先级、统一修复方案、35处修复全清单。home-override-system-scan-2026-06-16.md 补全第2轮(SOUL.md/.bashrc)和第3轮(20个skill文件)修复记录
  - 1.15.1: 修复 Python 路径 fallback 使用 expanduser("~/.hermes") 的 bug — HOME 重写时 `~` 解析错误，改为 pwd.getpwuid + os.path.join (被后续 1.16.0 纠正回社区标准)
  - 1.15.0: 新增 independent-checker 假阳性调度停跳诊断 pitfall；$HOME 重写陷阱增加 compliance-check.sh 场景示例
  - 1.14.0: 新增10b技能加载健康检查、10c全局SKILL版本一致性检查；修复规则条数检测bug；补充操作前置查证规则12a引用；新增disaster-recovery-restore.md参考；新增 $HOME 重写机制文档入驻 hermes-profile-architecture; path-standards.md 增加Python路径规范检查项
...
| ⑥ 纪律合规 | compliance-check 12项：身份/冷却期/版本锚/规则条数/脚本校验/记忆水位/内容锚点/路径绝对性/服务健康/能力归属（含10b技能加载健康+10c全局SKILL版本一致性）/外部记忆活性/治理合规 | `bash compliance-check.sh` |
...
`system-backup.sh` 每日凌晨5点自动运行（cron job `daily-backup`），备份以下数据到 `/mnt/d/Hermes/backups/system-snapshot/YYYY-MM-DD/`：
...
D 盘常驻镜像：`/mnt/d/Hermes/backups/audit/`（非快照版本，随时可查看）
...
**正常体积基线：** ~10,000 条消息 ≈ 100-150MB（约 12.5KB/条）。shici（200 条）≈ 2.5MB 可作为对照基线。curator 每 7 天自动清理超过 30 天的旧会话（`stale_after_days: 30`）。
...
# 所有应指向: /home/user/.hermes/mnemosyne
```
...
- 路径编写规范（含标准依据、社区 issue 索引）：`skill_view(name="system-guardian", file_path="references/path-standards.md")`
